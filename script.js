/* ===========================
   ...
   script.js - Cleaned Version (No Duplicates)
   =========================== */
...
const GROUP_LOAD_COOLDOWN = 300000;
...
function renderTemplatesByCategory(category) {
  ...
  updateTemplateLoadButtons(hasMore, hasLess, filteredTemplates.length);
}
...
function updateTemplateLoadButtons(hasMore, hasLess, totalCount) {
  const existingButtons = document.querySelector('.template-load-buttons');
  if (existingButtons) existingButtons.remove();
...
function updateGroupBanner() {
  ...
  if (banner) {
    banner.style.display = hasJoinedGroup() ? 'none' : 'block';
  }
}
...
async function startChatSequence(params) {
  ...
  let greeting = "Hello", greetingEmoji = "👋";
